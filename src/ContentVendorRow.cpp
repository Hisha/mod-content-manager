...
json ContentVendorServer::Objects(std::vector<ResolvedVendorRow> rows)
{
    ...
    for(auto const& r:rows) { j.push_back({{"package",r.packageKey},{"packageVersion",r.packageVersion},{"symbol",r.symbol},
        ...
        {"extendedCostId",r.costId},{"originalNpcFlags",r.originalFlags}}); if(r.flagsManaged)j.back()["templateManagesVendorFlag"]=true; }
    return j;
}
std::vector<ResolvedVendorRow> ContentVendorServer::Parse(json const& j)
{
    ...
    for(auto const& d:j)
    {
        if(!d.is_object()||(d.size()!=8&&d.size()!=9))throw std::runtime_error("Invalid resolved vendor fields");
        ...
        if(d.contains("templateManagesVendorFlag")){if(d.at("templateManagesVendorFlag")!=true)throw std::runtime_error("Invalid managed vendor flag marker");r.flagsManaged=true;}
