...
## Managed server deployment and immutable artifacts
...
The normal `.content activate <build>` workflow invokes the guarded server apply path before client publication when the build contains these definitions. The advanced `.content server apply <build>` command may run the same path independently, inserting definitions into the core's existing `itemextendedcost_dbc` overlay with a `content_manager_extended_cost_owner` snapshot. It participates in the same guarded world transaction as item/currency apply. Unowned rows cannot be adopted. Preflight and transactional guards protect against collisions and drift; failures roll back the world transaction and prevent client activation. Successful apply verifies exact definitions and provenance; worldserver restart is needed for SQL DBC loading. This infrastructure writes **no npc_vendor or event vendor rows** and does not create a Huntmaster consumer.
