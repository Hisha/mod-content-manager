...
Template declarations identify a stock `copyFrom` donor and a bounded `overrides` object. Descriptor v1 copies a reviewed set of runtime-relevant fields rather than exposing the full AzerothCore table. Creature overrides support `name`, `subname`, `minLevel`, `maxLevel`, `faction`, `npcFlags`, `unitFlags`, `typeFlags`, `aiName`, and `scriptName`. Gameobject overrides support `name`, `type`, `displayId`, `size`, `aiName`, and `scriptName`. Donors must exist when the build is composed.
