...
Server bundle/parity format 6 carries canonical managed-server representations. The guarded server apply path validates leases, donors, collisions, ownership and current snapshots, then applies templates, vendor relationships, and spawns in one InnoDB transaction. `.content activate <build-number>` invokes this path before client publication when managed rows are present; `.content server apply <build-number>` remains available as an advanced/manual command. `content_manager_server_resource_owner` records exact provenance. Any guard or SQL failure rolls back the complete apply and leaves the build `STAGED`.
